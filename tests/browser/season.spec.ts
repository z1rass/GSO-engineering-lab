...
test('Visitor sees a useful empty state in both languages when no Season is active', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Turn ideas into real things.' })).toBeVisible();
});
...
test('English remains selected when navigating from Season to About', async ({ page }) => {
  await page.goto('/season');
  await page.getByRole('button', { name: 'English' }).click();
  await page.getByRole('link', { name: 'About the Lab' }).click();
  await expect(page.locator('html')).toHaveAttribute('lang', 'en');
  await expect(page.getByRole('heading', { name: 'Take part. Make it yours.' })).toBeInViewport();
