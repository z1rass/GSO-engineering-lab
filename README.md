...
`activities` stores shared Activity data and a required single owner; `project_details` stores Project-specific fields. Creation and content updates are transactional. Links accept only HTTP(S), without embedded credentials. Text limits: title 120, goal 1,000, description 12,000, legacy materials/internal instructions 5,000 each; up to 30 tech-stack entries of 50 characters. New text materials belong in the description; existing materials are moved there by migration 0018. The JSON request ceiling is 128 KiB to accommodate multilingual content across these fields.
...
Planned date, end date and start/end times are optional. The editor chooses school (default), online or another place; another place needs a public general location. Dates are ISO calendar dates and times are `HH:mm` wall times in **Europe/Berlin**, not browser-local or UTC timestamps. An omitted end date means the same day when comparing known times. A supplied end date requires a start date and cannot precede it; on the same day the end time must follow the start. Multi-day Events may end at an earlier clock time on the later day. Unknown fields stay explicit in DE/EN, and plans are labelled unconfirmed until the owner opens registration. For a school Event, saving a complete date/start/end creates one Room Request automatically. The exact room comes from Ops confirmation.
...
Only the Activity owner opens registration with `POST /api/events/:id/open` and `{}`. Ops may edit Events but cannot open another owner's registration. PLANNING becomes ACTIVE (registration open) when date, start, end and public general location are present. Online and other-place Events open independently. School Events require a CONFIRMED Room Request whose slot covers the Event. The exact room is copied from the Ops confirmation, never entered in the Event editor. A confirmed booking cannot be discarded by changing the Event's place.
...
Event and Project pages share a Tasks panel. Activity Owner/Ops create and edit tasks in the Edit Event/Project management area, with a title, description and optional due date. Any current Member can take an OPEN task from its Activity page; this immediately assigns them and starts work. Taking a task never joins a Project or registers Going. The Task Owner or Activity Owner/Ops can complete it or release it back to OPEN through a task management menu; Activity Owner/Ops can also cancel unfinished tasks from Edit Event/Project. DONE preserves the responsible person's attribution. Terminal tasks cannot be taken or released.
...
The current owner enters a verified Member's school email in Edit Event/Project. The recipient gets an email link and confirms the transfer on a dedicated page within seven days. The current owner remains responsible until acceptance. The proposal and acceptance do not enroll either person in a Project/Event or reassign Tasks. The owner, recipient or Ops can cancel a pending proposal; Ops cannot force acceptance or propose in the owner's place. Departure without a successor uses Activity cancellation.

`GET /api/activities/:id/ownership` is Member-only; only the owner, recipient and Ops see the pending proposal. `POST` on the same path takes `{email}` and sends the invitation through the configured SMTP transport. `GET /api/ownership-invitations/:token` and `POST .../:token/accept` require the recipient's authenticated Member session; the database stores only a hash of the random token. `POST /api/activities/:id/ownership/cancel` takes `{transferId}`. Only PLANNING/ACTIVE Activities allow proposing/accepting. Cancelling a pending proposal remains available after closure.
...
Public `/seasons` and `/seasons/:id` pages show published Seasons and their Events/Projects. DRAFT Seasons are excluded even for direct public detail requests. `/` introduces the Lab to new visitors; `/home` shows the current Season and six newest PLANNING/ACTIVE Activities. These lists contain public fields only.

New Activities are linked to the ACTIVE Season automatically. If no Season is active, open Activities join when Ops activate the next one; open Projects also carry forward to each new Season without being copied. `GET /api/activities/:id/seasons` reads history. `activity_seasons` preserves previous links; team membership, Tasks and the Project ID stay unchanged. History badges list each actual Season, so participation in Season 0 and Season 2 never implies Season 1.
