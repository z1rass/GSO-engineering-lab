...
export function ProjectTeam({ id, language, closed=false }: {id:number;language:Language;closed?:boolean}) {
  ...
  return <section className="project-team" aria-label={t.title} aria-busy={resource.loading||busy}>
    ...
    {!data ? resource.loading ? <p role="status">{t.loading}</p> : <p role="alert">{t.loadError} <button className="text-link" onClick={reload}>{t.retry}</button></p>
      ...
      {data.members && <>{!closed&&<p className="field-hint">{t.hint}</p>}{!closed&&data.isOwner && <p className="ideas-note">{t.owner}</p>}{!data.canJoin && <p className="ideas-note">{t.closed}</p>}
