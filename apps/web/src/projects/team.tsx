...
const copy = {
  de: { title:'Team', join:'Projekt beitreten', joined:'Du bist Mitglied dieses Projektteams.', leave:'Team verlassen', hint:'Mach im Projekt mit. Dein Interesse bleibt davon unabhängig.', owner:'Du bist Owner dieses Projekts. Übertrage die Verantwortung, bevor du gehst.', closed:'Dieses Projekt nimmt keine neuen Mitglieder auf.', login:'Anmelden, um das Team zu sehen und mitzumachen', loading:'Team wird geladen …', saving:'Wird gespeichert …', error:'Die Änderung konnte nicht bestätigt werden. Bitte versuche es erneut.', loadError:'Das Team konnte nicht geladen werden.', conflict:'Das Projekt oder deine Verantwortung hat sich geändert. Lade das Team erneut.', retry:'Team neu laden', session:'Bitte melde dich erneut an.', signIn:'Anmelden', count:(n:number)=>`${n} im Team` },
  en: { title:'Team', join:'Join project', joined:'You are a member of this project team.', leave:'Leave team', hint:'Take part in the project. Your interest stays separate.', owner:'You own this project. Transfer responsibility before leaving.', closed:'This project is not accepting new members.', login:'Sign in to see the team and take part', loading:'Loading team …', saving:'Saving …', error:'Could not confirm the change. Please try again.', loadError:'Could not load the team.', conflict:'The project or your responsibility has changed. Reload the team.', retry:'Reload team', session:'Please sign in again.', signIn:'Sign in', count:(n:number)=>`${n} in the team` },
};
export function ProjectTeam({ id, language, closed=false }: {id:number;language:Language;closed?:boolean}) {
  ...
  return <section className="project-team" aria-label={t.title} aria-busy={resource.loading||busy}>
    ...
    {!data ? resource.loading ? <p role="status">{t.loading}</p> : <p role="alert">{t.loadError} <button className="text-link" onClick={reload}>{t.retry}</button></p>
      ...
      {data.members && <>{!closed&&<p className="field-hint">{t.hint}</p>}{data.joined&&<p className="participation-state">{t.joined}</p>}{!closed&&data.isOwner && <p className="ideas-note">{t.owner}</p>}{!data.canJoin && <p className="ideas-note">{t.closed}</p>}
