...
function TaskCard({task,id,language,done,manage}:{task:Task;id:number;language:Language;done:()=>void;manage:boolean}){
 ...
 return <article className="task-card" aria-label={task.title}><span className="status">{t[task.status]}</span><h3>{task.title}</h3>
  ...
  {!manage&&task.canTake&&<div className="task-public-actions"><button className="button-primary" disabled={state.busy} onClick={()=>void state.save(`/api/tasks/${task.id}/take`)}>{t.take}</button></div>}
  {(manage?(task.canEdit||task.canComplete||task.canCancel):task.canComplete&&!task.canEdit)&&<details className="project-form-section task-management"><summary>{manage?t.manage:t.myTask}</summary><div className="task-management-content">
    {task.canComplete&&<div className="account-actions"><button className="button-primary" disabled={state.busy} onClick={()=>void state.save(`/api/tasks/${task.id}/complete`)}>{t.complete}</button><button className="text-link" disabled={state.busy} onClick={()=>void state.save(`/api/tasks/${task.id}/release`)}>{t.release}</button></div>}
    {manage&&task.canEdit&&<details className="project-form-section"><summary>{t.edit}</summary><TaskForm id={id} task={task} language={language} done={done}/></details>}
    {manage&&task.canCancel&&<button className="button-danger" disabled={state.busy} onClick={()=>void state.save(`/api/tasks/${task.id}/cancel`)}>{t.cancel}</button>}
  </div></details>}{state.busy&&<p role="status">{t.busy}</p>}
  {state.error&&<p role="alert" className="form-error">{t[state.error]} <button className="text-link" onClick={done}>{t.retry}</button></p>}
 </article>;
}
export function TaskPanel({id,language,closed=false,manage=false}:{id:number;language:Language;closed?:boolean;manage?:boolean}){
