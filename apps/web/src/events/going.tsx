...
export function EventGoing({id,language,refresh}:{id:number;language:Language;refresh:()=>void}){
 ...
 return <section className="project-team" aria-label={t.title} aria-busy={busy||resource.loading}><h2>{t.title}</h2>
  ...
  :<><p role="status" aria-live="polite" aria-atomic="true">{t.count(data.count)}</p><p className="field-hint">{t.notice}</p>
   {!data.open&&<p className="ideas-note">{t.closed}</p>}
   {data.going?<button className="text-link" disabled={busy} onClick={()=>void act('leave')}>{busy?t.saving:t.leave}</button>:data.open&&(data.going===undefined?<Link className="text-link" to="/login">{t.login}</Link>:<button className="button-primary" disabled={busy} onClick={()=>void act('join')}>{busy?t.saving:t.join}</button>)}
   {data.isOwner&&!data.open&&<><p className="ideas-note">{t.ready}</p><button className="button-primary" disabled={busy||!data.canOpen} onClick={()=>void act('open')}>{busy?t.saving:t.open}</button></>}
