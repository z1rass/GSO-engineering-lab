  ...
  app.get('/api/me/activity', requireMember, async (request, response) => {
    ...
    const { rows } = await pool.query(`
      WITH participation AS (
        ...
        FROM activities a WHERE NOT a.hidden
      ...
      SELECT
        ...
        COALESCE((SELECT json_agg(t ORDER BY t."dueDate" NULLS LAST,t.id) FROM (
          ...
          FROM tasks t JOIN activities a ON a.id=t.activity_id WHERE t.owner_id=$1 AND NOT a.hidden
        ) t),'[]') AS tasks,
        json_build_object(
          'ideas',COALESCE((SELECT json_agg(i ORDER BY i.id DESC) FROM (
            SELECT i.id,i.title FROM ideas i JOIN idea_interests x ON x.idea_id=i.id WHERE x.user_id=$1 AND NOT i.hidden
          ) i),'[]'),
          'activities',COALESCE((SELECT json_agg(a ORDER BY a.id DESC) FROM (
            SELECT a.id,a.type,a.title,a.status FROM activities a JOIN activity_interests x ON x.activity_id=a.id WHERE x.user_id=$1 AND NOT a.hidden
          ) a),'[]')
