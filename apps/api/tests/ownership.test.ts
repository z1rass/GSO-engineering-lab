...
test('Ownership changes only when the named Member accepts; editing and leave follow the new owner for both Activity types',async()=>{
 ...
 for(const type of ['projects','events']){
  ...
  const proposed=await write(path,owner.cookie,{recipientId:successorId});expect(proposed.status).toBe(201);
  const transfer=(await proposed.json()).transfer;
  expect((await detail(successor.cookie))).toMatchObject({owner:before.owner,canEdit:false});
  expect((await write(`${path}/accept`,stranger.cookie,{transferId:transfer.id})).status).toBe(403);
  const attempts=await Promise.all([write(`${path}/accept`,successor.cookie,{transferId:transfer.id}),write(`${path}/accept`,successor.cookie,{transferId:transfer.id})]);
...
test('Only owner proposes; Ops or recipient can cancel, stale acceptance cannot consume a replacement, and private data stays private',async()=>{
 ...
 expect((await write(path,other.cookie,{recipientId:successorId})).status).toBe(403);
 expect((await write(path,ops.cookie,{recipientId:successorId})).status).toBe(403);
 ...
 expect(ownerState.candidates).toContainEqual(expect.objectContaining({id:successorId}));expect(JSON.stringify(ownerState)).not.toContain(successor.email);
 const first=(await write(path,owner.cookie,{recipientId:successorId}).then(r=>r.json())).transfer;
 expect((await write(path,owner.cookie,{recipientId:successorId})).status).toBe(409);
 expect((await fetch(`${base}${path}`,{headers:{cookie:other.cookie}}).then(r=>r.json()))).toEqual({pending:null,canPropose:false,candidates:[]});
 ...
 const second=(await write(path,owner.cookie,{recipientId:successorId}).then(r=>r.json())).transfer;
 expect((await write(`${path}/accept`,successor.cookie,{transferId:first.id})).status).toBe(409);
 const pending=(await fetch(`${base}${path}`,{headers:{cookie:successor.cookie}}).then(r=>r.json())).pending;
 expect(pending).toMatchObject({id:second.id,recipient:{id:successorId},canAccept:true,canCancel:true});
 expect((await write(`${path}/cancel`,successor.cookie,{transferId:second.id})).status).toBe(200);
 expect((await write(`${path}/accept`,successor.cookie,{transferId:second.id})).status).toBe(409);
});
test('Transfers reject self, Alumni, unverified recipients, closed Activities and unauthenticated or cross-origin actions',async()=>{
 ...
 const id=await activity(owner.cookie);const path=`/api/activities/${id}/ownership`;
 expect((await write(path,'',{recipientId})).status).toBe(401);
 expect((await write(path,owner.cookie,{recipientId},'POST','https://other.example')).status).toBe(403);
 expect((await write(path,owner.cookie,{recipientId:ownerId})).status).toBe(400);
 expect((await write(path,owner.cookie,{recipientId,ownerId})).status).toBe(400);
 await pool.query("UPDATE users SET affiliation='ALUMNI' WHERE id=$1",[recipientId]);
 expect((await write(path,owner.cookie,{recipientId})).status).toBe(400);
 ...
 const transfer=(await write(path,owner.cookie,{recipientId}).then(r=>r.json())).transfer;
 await pool.query("UPDATE users SET affiliation='ALUMNI' WHERE id=$1",[recipientId]);
 expect((await write(`${path}/accept`,successor.cookie,{transferId:transfer.id})).status).toBe(401);
 ...
 expect((await write(`${path}/accept`,successor.cookie,{transferId:transfer.id})).status).toBe(409);
 expect((await write(`${path}/cancel`,owner.cookie,{transferId:transfer.id})).status).toBe(200);
 expect((await write(path,owner.cookie,{recipientId})).status).toBe(409);
});
