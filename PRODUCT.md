...
The current code provides a public homepage and current Season page backed by PostgreSQL, German/English interface switching, responsive layouts and loading, empty and retry states. Local Compose supplies migrations and demonstration data. The shipped MVP slices include magic-link authentication/profile, Ideas, Events, Projects, Interested, Project membership, room requests, Going, Tasks, ownership transfer, and completion/cancellation with Past Activity views. Remaining Ops workflows, Season continuation and production operations stay planned.
