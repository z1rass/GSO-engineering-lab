...
The current code provides a public homepage and Season pages backed by PostgreSQL, German/English interface switching, responsive layouts and loading, empty and retry states. Local Compose supplies migrations and demonstration data. The shipped MVP slices include magic-link authentication/profile, Ideas, Events, Projects, Interested, Project membership, room requests, Going, Tasks, ownership transfer, completion/cancellation with Past Activity views, Ops Season management, Project continuation across Seasons, a private My Activity overview, Ops-only Club Network contacts, moderation with retained history, Ops profile deletion with anonymization and documented Ops recovery. Remaining production operations stay planned.
